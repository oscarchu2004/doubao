...
const AddPanoramaForm = ({ show, onClose, onPanoramaAdded, mapId, minimapPath }) => {
  ...
  const handleSubmit = async (e) => {
	...
	// NEW: ensure imagePath is absolute (not just /upload/... or data URI)
	const backendBase = "https://shome3-backend.hudini.online"; // or use an env variable
	if (createdPanorama.imagePath && !createdPanorama.imagePath.startsWith('http')) {
		createdPanorama.imagePath = `${backendBase}${createdPanorama.imagePath}`;
	}
            ...
            {showMinimapPicker && (
              <div className='picker-overlay d-flex justify-content-center align-items-center'>
                <div className='picker-content position-relative'>
                  ...
                  <img
                    src={getUploadedFileUrl(minimapPath)}
                    alt='Minimap picker'
